...
class Configuration(object):
    ...
    def add_atom(self, mol_id, typ, charge, coords, orientation=None):
        assert typ in self.atom_types
        atm_id = len(self.atoms) + 1
        self.atoms[atm_id] = {'mol_id': mol_id, 'type': typ, 'charge': charge,
    ...
    def append_atom_bonded(self, mol_id, typ, charge, len_bond, method, im1,
        ...
        self.atoms[atm_id] = {'mol_id': mol_id, 'type': typ, 'charge': charge, 'coords': ri}
        if orientation:
            self.atoms[atm_id]['orientation'] = np.copy(orientation[0:4])
        return atm_id
    ...
    def append_atom_unbonded(self, typ, charge, sep=None, orientation=None):
        ...
        self.atoms[atm_id] = {'mol_id': 0, 'type': typ, 'charge': charge, 'coords': ri}
        if orientation:
            self.atoms[atm_id]['orientation'] = np.copy(orientation[0:4])
        return atm_id
